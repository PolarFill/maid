...
config.read([f"{path}/Configurações/Principal.ini", 
                f"{path}/Configurações/session.info", 
                f"{path}/Configurações/TTS.ini"])
...
def google(text): #Função responsavel pela fala do gTTS
    ...
    if platform.system().startswith('W'):                  #Caso o sistema seja windows
        mp3_path = f"{path}\\Configurações\\tempfile.mp3"  #O diretório do mp3 será composto de \
    else:                                                  #Caso não seja windows
        mp3_path = f"{path}/Configurações/tempfile.mp3"    #O diretório do mp3 será composto de /
...
def watson(text): #Função responsavel pela fala do watson (ibm)
    ...
    if platform.system().startswith('W'):                                  #Caso o sistema seja windows
        audiopath = "{}\\Configurações\\tempfile.{}".format(path, formato) #O diretório do audio será composto de \
    else:                                                                  #Caso não seja windows
        audiopath = "{}/Configurações/tempfile.{}".format(path, formato)   #O diretório do audio será composto de /
